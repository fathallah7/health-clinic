...
const fetchTimeSlots = async () => {
    ...
    try {
        const response = await axios.get('admin/time-slots')
...
const submitForm = async () => {
    ...
    try {
        ...
        if (modalMode.value === 'create') {
            response = await axios.post('admin/time-slots', payload)
        } else {
            response = await axios.put(`admin/time-slots/${formData.value.id}`, payload)
        }
...
<template>
    <div class="p-6 bg-gray-50 min-h-screen">
        ...
        <!-- Modal -->
        <div v-if="showModal"
            class="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/50  bg-opacity-50 backdrop-blur-sm">
